...
dashboardRouter.get(
  ...
  wrap(async (req, res) => {
    ...
    const store = await query(`SELECT id, name FROM store WHERE id = $1`, [
      storeId,
    ]);
    ...
    const { rows: lowStock } = await query(
      `SELECT COUNT(*)::int AS items_below_min
         FROM store_product
        WHERE store_id = $1 AND qty <= min_qty`,
      [storeId],
    );
    ...
    res.json({
      ...
      items_below_min: lowStock[0].items_below_min,
    });
  }),
);
